...
void dec_weight_loader_buffer_qkvo_FFN(
    tapa::istream<hls::vector<ap_int<4>, DEC_QKVO_FFN_W_PARALLEL>>& weight_stream_in, 
    tapa::ostream<hls::vector<ap_int<4>, DEC_QKVO_FFN_W_PARALLEL>>& weight_stream_out, 
    int dec_seq_len
){
    decoder_seq_loop: for (int dec_seq_id = 0; dec_seq_id < dec_seq_len; dec_seq_id++){
        decoder_block_loop: for (int block_id = 0; block_id < DECODER_LAYER_NUM; block_id++){
            // dec_weight_loader_buffer<ap_int<4>, T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, INTER_DIM>(
            //     weight_stream_in, weight_stream_out, block_id, HIDDEN_DIM, KV_HIDDEN_DIM
            // );//for k
            ...
            dec_weight_loader_buffer<ap_int<4>, T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, INTER_DIM>(
                weight_stream_in, weight_stream_out, block_id, HIDDEN_DIM, KV_HIDDEN_DIM_PAD
            ); // for k and v, block 0~3: K, block 4~7: V

            dec_weight_loader_buffer<ap_int<4>, T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, INTER_DIM>(
                weight_stream_in, weight_stream_out, block_id, HIDDEN_DIM, HIDDEN_DIM
            );// for q
            ...
            dec_weight_loader_buffer<ap_int<4>, T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, INTER_DIM>(
                weight_stream_in, weight_stream_out, block_id, HIDDEN_DIM, INTER_DIM
            ); // for ffn_up
            ...
            dec_weight_loader_buffer<ap_int<4>, T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, INTER_DIM>(
                weight_stream_in, weight_stream_out, block_id, INTER_DIM, HIDDEN_DIM
            ); // for ffn_down
        }
        dec_weight_loader_buffer<ap_int<4>, T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, INTER_DIM>(
            weight_stream_in, weight_stream_out, 0, HIDDEN_DIM, VOCAB_SIZE_PAD
        ); // for vocab
    }
}
...
void dec_weight_s_loader_buffer_qkvo_FFN(
    tapa::istream<hls::vector<float, 2>>& weight_s_sum_stream_in,
    tapa::ostream<hls::vector<float, 2>>& weight_s_sum_stream_out,
    int dec_seq_len
){
    decoder_seq_loop: for (int dec_seq_id = 0; dec_seq_id < dec_seq_len; dec_seq_id++){
        decoder_block_loop: for (int block_id = 0; block_id < DECODER_LAYER_NUM; block_id++){
            // dec_weight_s_loader_buffer_fp32_bandwidth<T_QKVO_FFN_BLOCK_PARALLEL, true, INTER_DIM>(
            //     weight_s_sum_stream_in, weight_s_sum_stream_out, block_id, KV_HIDDEN_DIM
            // ); // for k
            ...
            dec_weight_s_loader_buffer_fp32_bandwidth<T_QKVO_FFN_BLOCK_PARALLEL, true, INTER_DIM>(
                weight_s_sum_stream_in, weight_s_sum_stream_out, block_id, KV_HIDDEN_DIM_PAD
            ); // for k and v, block 0~3: K, block 4~7: V

            dec_weight_s_loader_buffer_fp32_bandwidth<T_QKVO_FFN_BLOCK_PARALLEL, true, INTER_DIM>(
                weight_s_sum_stream_in, weight_s_sum_stream_out, block_id, HIDDEN_DIM
            ); // for q
            ...
            dec_weight_s_loader_buffer_fp32_bandwidth<T_QKVO_FFN_BLOCK_PARALLEL, true, INTER_DIM>(
                weight_s_sum_stream_in, weight_s_sum_stream_out, block_id, INTER_DIM
            ); // for ffn_up
        ...
        dec_weight_s_loader_buffer_fp32_bandwidth<T_QKVO_FFN_BLOCK_PARALLEL, true, INTER_DIM>(
            weight_s_sum_stream_in, weight_s_sum_stream_out, 0, VOCAB_SIZE_PAD
        ); // for vocab
    }
}
...
void dec_Linear_Layer_i4xi4_qkvo_FFN_flatten(
    ...
    decoder_seq_loop: for (int dec_seq_id = 0; dec_seq_id < dec_seq_len; dec_seq_id++){
        decoder_block_loop: for (int block_id = 0; block_id < DECODER_LAYER_NUM; block_id++){
            ...
            printf("Dec_seq_id %d: Block_id %d: Linear_Layer_i4xi4_qkvo_FFN_flatten completed for ffn_gate.\n", dec_seq_id, block_id);
...
void SpinQuant_Decoding(
    ...
    tapa::streams<hls::vector<ap_int<4>, DEC_QKVO_FFN_W_PARALLEL>, T_QKVO_FFN_BLOCK_PARALLEL> w_qkvo_ffn_buffer_streams("w_qkvo_ffn_buffer_streams");
    tapa::stream<hls::vector<float, 2>> w_s_sum_qkvo_ffn_stream("w_s_sum_qkvo_ffn_stream");
    tapa::stream<hls::vector<float, 2>> w_s_sum_qkvo_ffn_buffer_stream("w_s_sum_qkvo_ffn_buffer_stream");
    ...
    .invoke<tapa::detach, T_QKVO_FFN_BLOCK_PARALLEL>(dec_weight_loader_buffer_qkvo_FFN, w_qkvo_ffn_streams, w_qkvo_ffn_buffer_streams, dec_seq_len)
    .invoke(dec_weight_s_loader_qkvo_FFN, w_s_sum_qkvo_FFN_mmap, w_s_sum_qkvo_ffn_stream, dec_seq_len)
    .invoke(dec_weight_s_loader_buffer_qkvo_FFN, w_s_sum_qkvo_ffn_stream, w_s_sum_qkvo_ffn_buffer_stream, dec_seq_len)
    ...
    .invoke<tapa::detach, T_QKVO_FFN_BLOCK_PARALLEL>(
        dec_Linear_Layer_i4xi4_qkvo_FFN_flatten, quant_input_qkvo_ffn_loaders, w_qkvo_ffn_buffer_streams, quant_output_qkvo_ffn_drainers, dec_seq_len
    )
    ...
    .invoke(dec_dequant_layer_qkvo_FFN, quant_output_qkvo_ffn_stream, input_s_b_qkvo_ffn_stream, w_s_sum_qkvo_ffn_buffer_stream, output_qkvo_ffn_stream, dec_seq_len)
