...
void SpinQuant_Decoding_test(int argc, char* argv[]) {
    ...
    // for (int i = 0; i < T_QKVO_FFN_BLOCK_PARALLEL/2; i++) {
    //     initInt8Vec(w_qkvo_FFN_mmaps_half_0_k_caches[i], DEC_QKVO_FFN_W_PARALLEL/2);
    // }
    // for (int i = 0; i < T_QKVO_FFN_BLOCK_PARALLEL/2; i++) {
    //     initInt8Vec(w_qkvo_FFN_mmaps_half_1_v_caches[i], DEC_QKVO_FFN_W_PARALLEL/2);
    // }
            ...
            for(int layer = 0; layer < DECODER_LAYER_NUM; layer++){
                for (int h = 0; h < KV_HEAD_NUM; h++) {
                    ...
                    for (int j = 0; j < HEAD_DIM; j++) {
                        ...
                        int k_idx = ((layer * KV_HEAD_NUM/DEC_HEAD_PARALLEL + h % (KV_HEAD_NUM/DEC_HEAD_PARALLEL)) * MAX_SUM_SEQ_LEN + i)/DEC_K_PARALLEL *
                                    HEAD_DIM + j;
                        int k_sub_idx = i % DEC_K_PARALLEL;
                        if(i < MAX_PRE_SEQ_LEN){
                            // k_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][k_idx][k_sub_idx] = ap_int<8>(qval_k);
                            w_qkvo_FFN_mmaps_half_0_k_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][w_qkvo_FFN_size + k_idx][k_sub_idx] = ap_int<8>(qval_k);
                        }
                        else {
                            // k_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][idx][sub_idx] = ap_int<8>(0);
                            w_qkvo_FFN_mmaps_half_0_k_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][w_qkvo_FFN_size + k_idx][k_sub_idx] = ap_int<8>(0);
                        }

                        int qval_v = std::round(distF(rng) / scale);
                        qval_v = std::max(-128, std::min(127, qval_v));
                        int v_idx = ((layer * (KV_HEAD_NUM/DEC_HEAD_PARALLEL) + h % (KV_HEAD_NUM/DEC_HEAD_PARALLEL)) * HEAD_DIM + j)/DEC_V_PARALLEL * MAX_SUM_SEQ_LEN + i;
                        int v_sub_idx = j % DEC_V_PARALLEL;
                        if(i < MAX_PRE_SEQ_LEN){
                            // v_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][idx][sub_idx] = ap_int<8>(qval_v);
                            w_qkvo_FFN_mmaps_half_1_v_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][w_qkvo_FFN_size + v_idx][v_sub_idx] = ap_int<8>(qval_v);
                        }
                        else {
                            // v_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][idx][sub_idx] = ap_int<8>(0);
                            w_qkvo_FFN_mmaps_half_1_v_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][w_qkvo_FFN_size + v_idx][v_sub_idx] = ap_int<8>(0);
                        }
                    }
                }
            }
