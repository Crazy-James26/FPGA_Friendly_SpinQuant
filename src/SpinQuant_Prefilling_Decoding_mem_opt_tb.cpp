...
void prefilling_read_int4_bin_as_blocked_int8_weight_mmaps(
    ...
    for (int n = 0; n < output_dim / 2; ++n) {
        ...
        if (!fin) { std::cerr << "Read error @ row " << (2*n+1) << " in " << name << "\n"; return; }
            ...
            std::fill(data_1.begin(), data_1.end(), 0);
        }
                ...
                for (int h = 0; h < KV_HEAD_NUM; h++) {
                    for (int j = 0; j < HEAD_DIM; j++) {
                        ...
                        if(i < MAX_PRE_SEQ_LEN){
                            ...
                            w_qkvo_FFN_mmaps_half_0_k_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][w_qkvo_FFN_size + write_idx][write_sub_idx] = ap_int<8>(0);
                        }
                        ...
                        if(i < MAX_PRE_SEQ_LEN){
                            ...
                            w_qkvo_FFN_mmaps_half_1_v_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][w_qkvo_FFN_size + write_idx][write_sub_idx] = ap_int<8>(0);
                        }
                    }
                }
            }
