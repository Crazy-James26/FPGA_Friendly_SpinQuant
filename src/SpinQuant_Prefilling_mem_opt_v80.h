...
void pref_dequant_layer_kq_int_fp32(
    ...
    decoder_block_loop: for (int block_id = 0; block_id < DECODER_LAYER_NUM; block_id++){
        pref_dequant_layer_qint_fp32<log2_HIDDEN_DIM + 8, true, TOKEN_PARALLEL, 2, HIDDEN_DIM>(
...
void pref_RoPE_layer_kq(
    ...
    decoder_block_loop: for (int block_id = 0; block_id < DECODER_LAYER_NUM; block_id++){
        pref_RoPE_layer<float, TOKEN_PARALLEL, HEAD_DIM>(input_stream_kq, output_stream_kq, KV_HEAD_NUM, seq_len);
        cout << "Block_id: " << block_id << " RoPE_layer for k completed." << endl;
        pref_RoPE_layer<float, TOKEN_PARALLEL, HEAD_DIM>(input_stream_kq, output_stream_kq, Q_HEAD_NUM, seq_len);
...
void pref_weight_loader_buffer_w_ffn_gate(
    tapa::istream<hls::vector<ap_int<4>, PRE_FFN_W_PARALLEL/PRE_FFN_W_BLOCK_NUM>>& w_ffn_gate_stream_in, 
    tapa::ostream<hls::vector<ap_int<4>, PRE_FFN_W_PARALLEL/PRE_FFN_W_BLOCK_NUM>>& w_ffn_gate_stream_out, 
    int seq_len
){
    decoder_block_loop: for (int block_id = 0; block_id < DECODER_LAYER_NUM; block_id++){
        pref_weight_loader_buffer<ap_int<4>, PRE_FFN_W_PARALLEL/PRE_FFN_W_BLOCK_NUM, TOKEN_PARALLEL, HIDDEN_DIM, INTER_DIM/PRE_FFN_W_BLOCK_NUM>(
            w_ffn_gate_stream_in, w_ffn_gate_stream_out, block_id, seq_len, HIDDEN_DIM, INTER_DIM/PRE_FFN_W_BLOCK_NUM
        );
    }
}
...
void pref_weight_s_loader_buffer_w_ffn_gate(
    tapa::istream<hls::vector<float, 2>>& w_ffn_gate_s_sum_stream_in,
    tapa::ostream<hls::vector<float, 2>>& w_ffn_gate_s_sum_stream_out,
    int seq_len
){
    decoder_block_loop: for (int block_id = 0; block_id < DECODER_LAYER_NUM; block_id++){
        pref_weight_s_loader_buffer_fp32<TOKEN_PARALLEL, true, INTER_DIM>(
            w_ffn_gate_s_sum_stream_in, w_ffn_gate_s_sum_stream_out, block_id, seq_len, INTER_DIM
        );
    }
}
...
void SpinQuant_Prefilling(
    ...
    tapa::streams<hls::vector<ap_int<4>, PRE_FFN_W_PARALLEL/PRE_FFN_W_BLOCK_NUM>, PRE_FFN_W_BLOCK_NUM> w_ffn_gate_buffer_streams("w_ffn_gate_buffer_streams");
    tapa::stream<hls::vector<float, 2>> w_ffn_gate_s_sum_stream("w_ffn_gate_s_sum_stream");
    tapa::stream<hls::vector<float, 2>> w_ffn_gate_s_sum_buffer_stream("w_ffn_gate_s_sum_buffer_stream");
    ...
    .invoke<tapa::detach, PRE_FFN_W_BLOCK_NUM>(pref_weight_loader_buffer_w_ffn_gate, w_ffn_gate_streams, w_ffn_gate_buffer_streams, seq_len)
    .invoke(pref_weight_s_loader_w_ffn_gate, w_ffn_gate_s_sum_mmap, w_ffn_gate_s_sum_stream, seq_len)
    .invoke(pref_weight_s_loader_buffer_w_ffn_gate, w_ffn_gate_s_sum_stream, w_ffn_gate_s_sum_buffer_stream, seq_len)
    // .invoke(Linear_Layer_i4xi4_ffn_gate, quant_ln_res0_stream_ffn_gate, w_ffn_gate_streams, quant_ffn_gate_stream, seq_len)
    .invoke(pref_Linear_Layer_i4xi4_ffn_gate, quant_ln_res0_stream_ffn_gate, w_ffn_gate_buffer_streams, quant_ffn_gate_stream_redundant, seq_len)
    .invoke(pref_ffn_gate_discard, quant_ffn_gate_stream_redundant, quant_ffn_gate_stream, seq_len)
    .invoke(pref_dequant_layer_ffn_gate_int_fp32, quant_ffn_gate_stream, ln_res0_s_b_stream_ffn_gate, w_ffn_gate_s_sum_buffer_stream, ffn_gate_stream, seq_len)
