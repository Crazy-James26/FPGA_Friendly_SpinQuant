...
void dec_dequant_layer_qint_fp32_bandwidth(
    ...
    #pragma HLS ARRAY_PARTITION variable=weight_s type=cyclic factor=block_parallel dim=1
    float weight_sum[block_parallel * weight_parallel];
    #pragma HLS ARRAY_PARTITION variable=weight_sum type=cyclic factor=block_parallel dim=1
    output_dim_loop: for(int k = 0; k < output_dim/(block_parallel * weight_parallel); k++){
        ...
        dequant_weight_loop: for(int n = 0; n < weight_parallel; n++){
            ...
            #pragma HLS unroll
                if(is_act_asym){
                    float dequant_temp = temp_pack[m] * s * weight_s[n * block_parallel + m];
                    output_pack[m] = dequant_temp + b * weight_sum[n * block_parallel + m];
                }
                else{
                    float dequant_temp = temp_pack[m] * s * weight_s[n * block_parallel + m];
                    output_pack[m] = dequant_temp;
